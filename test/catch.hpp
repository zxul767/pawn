...
// generated methods CATCH_CONFIG_CPP11_IS_ENUM : std::is_enum is supported?
...
std::vector<TestCase> filterTests(
    std::vector<TestCase> const &testCases, TestSpec const &testSpec,
...
void registerTestCase(
    ITestCase *testCase, char const *className, NameAndDesc const &nameAndDesc,
...
struct AutoReg
{
    ...
    template <typename C>
    AutoReg(
        void (C::*method)(), char const *className, NameAndDesc const &nameAndDesc,
        SourceLineInfo const &lineInfo)
    {
...
struct AssertionInfo
{
    ...
    AssertionInfo(
        std::string const &_macroName, SourceLineInfo const &_lineInfo,
...
inline Impl::StdString::Contains Contains(
    std::string const &substr, CaseSensitive::Choice caseSensitivity = CaseSensitive::Yes)
{
...
class ResultBuilder
{
  public:
    ResultBuilder(
        ...
        char const *capturedExpression, ResultDisposition::Flags resultDisposition,
...
{
    MessageBuilder(
        std::string const &macroName, SourceLineInfo const &lineInfo,
        ResultWas::OfType type)
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                          \
    ...
    {                                                                                    \
        ...
        Catch::isTrue(false && (expr))) // expr here is never evaluated at runtime but it
    ...
    {                                                                                    \
        ...
        {                                                                                \
            ...
            __catchResult.setLhs(Catch::toString(arg))                                   \
                .setRhs(                                                                 \
                    matcherAsString == Catch::Detail::unprintableString                  \
                        ? #matcher                                                       \
                        : matcherAsString)                                               \
                .setOp("matches")                                                        \
...
{
    SectionEndInfo(
        SectionInfo const &_sectionInfo, Counts const &_prevAssertions,
...
template <typename T> class CompositeGenerator
{
  ...
  private:
    ...
    {
        std::copy(
            other.m_composed.begin(), other.m_composed.end(),
...
struct IMutableRegistryHub
{
    ...
    virtual void registerReporter(
        std::string const &name, Ptr<IReporterFactory> const &factory) = 0;
...
TestCase makeTestCase(
    ITestCase *testCase, std::string const &className, std::string const &name,
...
struct ConfigData
{
    ...
    ConfigData()
        : listTests(false), listTags(false), listReporters(false),
          listTestNamesOnly(false), showSuccessfulTests(false), shouldDebugBreak(false),
          noThrow(false), showHelp(false), showInvisibles(false), forceColour(false),
          filenamesAsTags(false), abortAfter(-1), rngSeed(0),
...
struct Parser
{
    ...
    void parseIntoTokens(
        int argc, char const *const *argv, std::vector<Parser::Token> &tokens) const
    {
...
template <typename ConfigT> class CommandLine
{
    ...
    class ArgBuilder
    {
      public:
        ...
        // Bind a method taking a single, non-boolean argument (requires a placeholder
        // string)
        template <typename C, typename M>
  ...
  public:
    ...
    std::vector<Parser::Token> parseInto(
        int argc, char const *const *argv, ConfigT &config) const
    {
...
{
    SectionStats(
        SectionInfo const &_sectionInfo, Counts const &_assertions,
...
{
    TestCaseStats(
        TestCaseInfo const &_testInfo, Totals const &_totals, std::string const &_stdOut,
...
inline std::size_t listTags(Config const &config)
{
         ...
         countIt != countItEnd; ++countIt)
    {
        ...
        Text wrapper(
            countIt->second.all(), TextAttributes()
                                       .setInitialIndent(0)
...
class RunContext : public IResultCapture, public IRunner
{
  ...
  public:
    explicit RunContext(
        Ptr<IConfig const> const &_config, Ptr<IStreamingReporter> const &reporter)
    ...
    void testGroupEnded(
        std::string const &testSpec, Totals const &totals, std::size_t groupIndex,
  ...
  private: // IResultCapture
    ...
    virtual void sectionEnded(SectionEndInfo const &endInfo)
    {
        ...
        m_reporter->sectionEnded(SectionStats(
            endInfo.sectionInfo, assertions, endInfo.durationInSeconds,
            missingAssertions));
        m_messages.clear();
    }
  ...
  private:
    ResultBuilder makeUnexpectedResultBuilder() const
    {
        return ResultBuilder(
            m_lastAssertionInfo.macroName.c_str(), m_lastAssertionInfo.lineInfo,
...
Ptr<IStreamingReporter> createReporter(
    std::string const &reporterName, Ptr<Config> const &config)
{
...
Ptr<IStreamingReporter> addListeners(
    Ptr<IConfig const> const &config, Ptr<IStreamingReporter> reporters)
{
...
void registerTestCase(
    ITestCase *testCase, char const *classOrQualifiedMethodName,
    NameAndDesc const &nameAndDesc, SourceLineInfo const &lineInfo)
{

    getMutableRegistryHub().registerTest(makeTestCase(
        testCase, extractClassName(classOrQualifiedMethodName), nameAndDesc.name,
        nameAndDesc.description, lineInfo));
}
void registerTestCaseFunction(
    TestFunction function, SourceLineInfo const &lineInfo, NameAndDesc const &nameAndDesc)
{
...
class RegistryHub : public IRegistryHub, public IMutableRegistryHub
{
  ...
  public: // IMutableRegistryHub
    virtual void registerReporter(
        std::string const &name, Ptr<IReporterFactory> const &factory) CATCH_OVERRIDE
    {
...
TestCase makeTestCase(
    ITestCase *_testCase, std::string const &_className, std::string const &_name,
...
TestCaseInfo::TestCaseInfo(TestCaseInfo const &other)
    : name(other.name), className(other.className), description(other.description),
      tags(other.tags), lcaseTags(other.lcaseTags), tagsAsString(other.tagsAsString),
...
struct IReporter : IShared
{
    ...
    virtual void EndTestCase(
        TestCaseInfo const &testInfo, Totals const &totals, std::string const &stdOut,
        std::string const &stdErr) = 0;
    virtual void StartSection(
        std::string const &sectionName, std::string const &description) = 0;
...
void LegacyReporterAdapter::testCaseEnded(TestCaseStats const &testCaseStats)
{
    m_legacyReporter->EndTestCase(
        testCaseStats.testInfo, testCaseStats.totals, testCaseStats.stdOut,
...
ResultBuilder::ResultBuilder(
    char const *macroName, SourceLineInfo const &lineInfo, char const *capturedExpression,
...
template <typename T> class ReporterRegistrar
{
    ...
    class ReporterFactory : public SharedImpl<IReporterFactory>
    {
        ...
        // register your custom reporter class be aware that the native reporter interface
        // has changed to IStreamingReporter. The "legacy" interface, IReporter, is still
        // supported via an adapter. Just use REGISTER_LEGACY_REPORTER to take advantage
        // of the adapter. However please consider updating to the new interface as the
        // old one is now deprecated and will probably be removed quite soon! Please
        // contact me via github if you have any questions at all about this. In fact,
        // ideally, please contact me anyway to let me know you've hit this - as I have no
        // idea who is actually using custom reporters at all (possibly no-one!). The new
        // interface is designed to minimise exposure to interface changes in the future.
        virtual IStreamingReporter *create(ReporterConfig const &config) const
        {
            return new T(config);
        }
...
class JunitReporter : public CumulativeReporterBase
{
  public:
    ...
    void writeSection(
        std::string const &className, std::string const &rootName,
...
#define CATCH_CHECK_THROWS_WITH(expr, matcher)                                           \
    INTERNAL_CATCH_THROWS(                                                               \
        expr, Catch::ResultDisposition::ContinueOnFailure, matcher,                      \
...
#define CATCH_WARN(msg)                                                                  \
    INTERNAL_CATCH_MSG(                                                                  \
        Catch::ResultWas::Warning, Catch::ResultDisposition::ContinueOnFailure,          \
        "CATCH_WARN", msg)
...
#define CATCH_SUCCEED(...)                                                               \
    INTERNAL_CATCH_MSG(                                                                  \
        Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure,               \
...
#define REQUIRE_THROWS_WITH(expr, matcher)                                               \
    INTERNAL_CATCH_THROWS(                                                               \
        expr, Catch::ResultDisposition::Normal, matcher, "REQUIRE_THROWS_WITH")
...
#define WARN(msg)                                                                        \
    INTERNAL_CATCH_MSG(                                                                  \
        Catch::ResultWas::Warning, Catch::ResultDisposition::ContinueOnFailure, "WARN",  \
        msg)
...
#define FAIL(...)                                                                        \
    INTERNAL_CATCH_MSG(                                                                  \
        Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, "FAIL",     \
        __VA_ARGS__)
#define SUCCEED(...)                                                                     \
    INTERNAL_CATCH_MSG(                                                                  \
        Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, "SUCCEED",    \
        __VA_ARGS__)
#else
