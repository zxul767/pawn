...
class AlphaBetaSearch : public IEngine
{
  private:
    ...
    int iterative_deepening_search(
        int depth, std::vector<game_rules::Move> &principal_variation);
    int evaluate_position(const game_rules::IBoard *board);

    bool build_principal_variation(
        game_rules::IBoard *, std::vector<game_rules::Move> &principal_variation);
  ...
  public:
    ...
    GameResult get_best_move(
        int depth, game_rules::IBoard *, game_rules::Move &best_move);
};
